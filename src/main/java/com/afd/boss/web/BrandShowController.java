...
public class BrandShowController {
	...
	public String manList(HttpServletRequest request) {
		// 处理分页信息
		PageInfo pageInfo = null;
		...
		if (StringUtils.isNotBlank(pageInfo.getConditions().get("status"))) {
			cond.put("status", pageInfo.getConditions().get("status"));
		}

		Page<BrandShow> brandShowPage = brandShowService.queryBrandShowByPage(
				cond, pageInfo.getPageNo());
		request.setAttribute("brandShowPage", brandShowPage);

		return "/brandShow/manList";
	}
