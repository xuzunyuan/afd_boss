		...
		<script type="text/javascript">
			$(function(){
				...
				$(document).on("change","input[name=orderCheck]",function(){
					...
					$("input[name=allCheck]").prop("checked",allCheck);
				});
				$(document).on("click","input[name=updateStatus]",function(){
					var newStatus = $("select[name=newOrderStatus]").children("option:selected").val();
					if(newStatus == '-1'){
						alert("请选择要更新的状态！");
						return;
					}
					...
					$.post(
						"${ctx}/order/chgOrderStatus",
						{newOrderStatus:newStatus,orderIds:checkOrderIds.join(",")},
						function(json){
							if(json.success){
								alert("更新成功！");
							}else{
